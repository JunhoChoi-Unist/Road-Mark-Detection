...
def walkThroughDataset(dataSetDir):
    ...
    for imageFile in tqdm(listOfFiles):
        ...
        if vpPoint[0] > 1 and vpPoint[1] > 1:
            ...
            cv2.circle(
                segImage, vpPoint, 15, (c[0], c[1], c[2]), thickness=-1, lineType=8
            )
...
def parse_args():
    parser = argparse.ArgumentParser(description="VPGNet Dataset Quick Inspector")
    parser.add_argument(
        "--rootDir",
        type=str,
        default=r"D:\VPGNet-DB-5ch",
