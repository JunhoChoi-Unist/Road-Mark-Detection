...
def draw_vpp(model, dataset, num=10):
    ...
    for n in range(num):
        ...
        axs[n,0].imshow(quads[n].repeat_interleave(4,1).repeat_interleave(4,0), alpha=0.5)
        axs[n,0].axis('off')
        axs[n,1].imshow(rgbs[n].permute(1,2,0))
        axs[n,1].imshow(quad_hats[n].repeat_interleave(4,1).repeat_interleave(4,2).squeeze(0), alpha=0.5)
        axs[n,1].axis('off')
    plt.show()
